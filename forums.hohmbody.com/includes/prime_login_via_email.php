		...
		function correct_username_var($admin)
		{
			...
			if (!empty($config['login_via_email_enable']))
			{
				...
				if ($admin && isset($_POST['login']))
				{
					...
					{
						$request->overwrite('username', $user->data['username'], \phpbb\request\request_interface::POST);
						$request->overwrite('username', $user->data['username'], \phpbb\request\request_interface::REQUEST);
					}
				}
